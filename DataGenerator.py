...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for i, idx in enumerate(indexes):
            ...
            last_iq = first_iq + self.input_size
            ...
            X[i, ] = np.reshape(
                self.iq[first_iq:last_iq],
                (self.how_many_blocks_per_frame, self.input_size, 2)
            )
            ...
            # X[i, ] = np.reshape(
            #     self.iq[
            #         (idx * self.num_blocks_per_frame * self.num_samples_per_block):
            #         ((idx + 1) * self.num_blocks_per_frame * self.num_samples_per_block)],
            #     (self.num_blocks_per_frame, self.num_samples_per_block, 2))
            #

            # tx_beam_samples = self.tx_beam[(idx * self.num_blocks_per_frame * self.num_samples_per_block):
            #                                ((idx + 1) * self.num_blocks_per_frame * self.num_samples_per_block)]
